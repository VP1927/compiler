...
void scan(void) {   //move one token ahead
...
void eat(TokenType tokenType) {    //check if token is tokenType, if true, call scan func to move to the next token, else error
...
void compileTypeDecl(void) {
  assert("Parsing Type Declaration ....");
  eat(TK_IDENT);
  eat(SB_EQ);
  compileType();
  eat(SB_SEMICOLON);
...
void compileVarDecl(void) {
  assert("Parsing Variable Declaration ....");
  eat(TK_IDENT);
  eat(SB_COLON);
  compileType();
  eat(SB_SEMICOLON);
